...
from Related_Codes.DECAutoencoder import ClusteringLayer
from sklearn.metrics import adjusted_rand_score, normalized_mutual_info_score

class GenericIDECAutoencoder(nn.Module):

    def __init__(self, device, n_clusters, input_dim, latent_dim, negative_slope):
        super(GenericIDECAutoencoder, self).__init__()
        self.device = device
    ...
    def set_training_variables(self, n_epochs, lr, momentum, alpha, gamma):
    ...
    def train_on_batch(self, x, p, optimizer):
        optimizer.zero_grad()
        q, _ = self(x)
        loss = torch.sum(p * torch.log(p / (q + 1e-6) + 1e-6))
        loss.backward()
        optimizer.step()
        return loss.item()
    ...
    def train_autoencoder(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch_index, (data, labels) in enumerate(self.dataloader):
                ...
                rec_loss = MSE(reconstructions, data)
                clustering_loss = KLDivLoss(output.log(), target) / output.shape[0]
                
                total_loss = rec_loss + self.gamma * clustering_loss 
                
                sum_rec_loss += rec_loss.item()
                sum_clustering_loss += self.gamma * clustering_loss.item()
            ...
            self.df_eval.loc[epoch] = [sum_rec_loss, sum_clustering_loss, acc, pur, nmi, ari]
            print(f'Ep: {epoch} Rec L: {sum_rec_loss:.4f} Cl L: {sum_clustering_loss:.4f} ACC: {acc:.2f} PUR: {pur:.2f} NMI: {nmi:.2f} ARI: {ari:.2f}')
    ...
    def set_path(self):
        ...
        self.properties_name += '_momentum_' + str(self.momentum) + '_alpha_' + str(self.alpha) + '_gamma_' + str(self.gamma)
        self.data_dir_path = self.path_to_module + 'Results/' + self.dataset_name + '/IDECAE/' + self.properties_name

class IDECAutoencoder(GenericIDECAutoencoder):
    def __init__(self,  device, n_clusters, input_dim, latent_dim, negative_slope):
        super(IDECAutoencoder, self).__init__( device, n_clusters, input_dim, latent_dim, negative_slope)
