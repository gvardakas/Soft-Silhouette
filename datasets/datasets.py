...
def get_dermatology_np():
	na_column = 33
	labels_column = 34
	df = pd.read_csv("/content/drive/MyDrive/Colab_Notebooks/datasets/Dermatology/dermatology.data", delimiter=",", header=None, na_values="?")
...
def get_ecoil_np():
	...
	df = pd.read_csv("/content/drive/MyDrive/Colab_Notebooks/datasets/Ecoli/ecoli.data", delimiter="\s+", header=None, names=column_names)
...
def get_olivetti_faces_np():
	data_home = "/content/drive/MyDrive/Colab_Notebooks/datasets/Olivetti-Faces"
...
def get_australian_np():
	...
	df = pd.read_csv("/content/drive/MyDrive/Colab_Notebooks/datasets/Australian/australian.dat", delimiter=" ", header=None, names=column_names)
...
def get_wine_np():
	...
	df = pd.read_csv("/content/drive/MyDrive/Colab_Notebooks/datasets/Wine/wine.data", header=None, names=column_names)
...
def get_iris_np():
	...
	df = pd.read_csv("/content/drive/MyDrive/Colab_Notebooks/datasets/Iris/iris.data", header=None, names=columnn_names)
	data = df[["f1", "f2", "f3", "f4"]]
	labels = df["label"]
...
def get_ghouls_goblins_and_ghosts_np():
	columnn_names = ["f1", "f2", "f3", "f4","f5","f6", "label"]
	df = pd.read_csv("/content/drive/MyDrive/Colab_Notebooks/datasets/Ghouls_Goblins_And_Ghosts/Ghouls_Goblins_And_Ghosts.csv", header=None, names=columnn_names)
	data = df[["f2", "f3","f4","f5","f6"]]
	labels = df["label"]
...
def get_ghouls_goblins_and_ghosts_dataloader(batch_size=50):
	data, labels = get_ghouls_goblins_and_ghosts_np()
...
def get_synthetic_np():
	data = np.load("/content/drive/MyDrive/Colab_Notebooks/datasets/Synthetic/synthetic.npy")
	labels = np.load("/content/drive/MyDrive/Colab_Notebooks/datasets/Synthetic/synthetic_labels.npy")
	total_size = data.shape[0]
...
def get_10x_73k_np(data_points=-1):
	...
	data_path = "/content/drive/MyDrive/Colab_Notebooks/datasets/10x_73k/sub_set-720.mtx"
	labels_path = "/content/drive/MyDrive/Colab_Notebooks/datasets/10x_73k/labels.txt"
...
def get_MNIST_dataloader(batch_size=64):
	...
	train = datasets.MNIST("/content/drive/MyDrive/Colab_Notebooks/datasets/MNIST", train=True, download=True, transform=transform)
...
def get_FashionMNIST_dataloader(batch_size=64):
	transform = transforms.Compose([transforms.Resize(IMG_SIZE), transforms.ToTensor()])
	train = datasets.FashionMNIST("/content/drive/MyDrive/Colab_Notebooks/datasets/FashionMNIST", train=True, download=True, transform=transform)
	dataloader = torch.utils.data.DataLoader(train, batch_size=batch_size, drop_last=drop_last, shuffle=shuffle)
	return dataloader

def get_FashionMNIST_5_dataloader(batch_size=64):
	data = np.load("/content/drive/MyDrive/Colab_Notebooks/datasets/FashionMNIST_5/fashionmnist_5.npy")
	labels = np.load("/content/drive/MyDrive/Colab_Notebooks/datasets/FashionMNIST_5/fashionmnist_5_labels.npy")
...
def get_Pendigits_np():
	data = np.loadtxt("/content/drive/MyDrive/Colab_Notebooks/datasets/Pendigits/pendigits.tra.txt", delimiter=',')
...
def get_MNIST_subset_np(create_subset=False, data_per_pattern=1000):
	if(create_subset):
		...
		np.save("/content/drive/MyDrive/Colab_Notebooks/datasets/MNIST/MNIST_subset/MNIST.npy", data)
		np.save("/content/drive/MyDrive/Colab_Notebooks/datasets/MNIST/MNIST_subset/MNIST_labels.npy", labels)

	# Load data
	data = np.load("/content/drive/MyDrive/Colab_Notebooks/datasets/MNIST/MNIST_subset/MNIST.npy")
	labels = np.load("/content/drive/MyDrive/Colab_Notebooks/datasets/MNIST/MNIST_subset/MNIST_labels.npy")
...
def get_MNIST_subset_dataloader(batch_size=64, create_subset=False, data_per_pattern=1000):
	...
	final_dataset = TensorDataset(data, labels)
	dataloader = DataLoader(final_dataset, batch_size=batch_size, drop_last=drop_last, shuffle=shuffle)
	return dataloader,channel_number
	...
	if(create_subset):
		...
		for datapoint in enumerate(trainset):
			...
			if example_targets in dict_counter and dict_counter[example_targets] < data_per_pattern:
				...
				data.append(np.array(example_data))
				labels.append(example_targets)
		np.save("/content/drive/MyDrive/Colab_Notebooks/datasets/FashionMNIST/FashionMNIST_subset/FashionMNIST.npy", data)
		np.save("/content/drive/MyDrive/Colab_Notebooks/datasets/FashionMNIST/FashionMNIST_subset/FashionMNIST_labels.npy", labels)

	# Load data
	data = np.load("/content/drive/MyDrive/Colab_Notebooks/datasets/FashionMNIST/FashionMNIST_subset/FashionMNIST.npy")
	labels = np.load("/content/drive/MyDrive/Colab_Notebooks/datasets/FashionMNIST/FashionMNIST_subset/FashionMNIST_labels.npy")
